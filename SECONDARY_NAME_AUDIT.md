...
- **Duotone icons checked across all packs:** 20,989
- **Aliased (cmap → wrong glyph name):** 3,344
...
|---|---|---:|---:|---:|---|---|
| `twemoji` | `TwemojiSecondary` | 4,498 | 1,127 | 0 | `a-button-blood-type`→`a-button`, `ab-button-blood-type`→`ab-button` | — |
| `pepicons-print` | `PepiconsPrintSecondary` | 703 | 444 | 0 | `airplane-circle-off`→`airplane-circle`, `airplane-off`→`airplane` | — |
| `circle-flags` | `CircleFlagsSecondary` | 732 | 348 | 0 | `afar`→`aa`, `bg`→`av` | — |
...
| `fluent-emoji-flat` | `FluentEmojiFlatSecondary` | 495 | 164 | 0 | `backhand-index-pointing-down-light`→`backhand-index-pointing-down-dark`, `backhand-index-pointing-down-medium`→`backhand-index-pointing-down-dark` | — |
| `glyphs` | `GlyphsSecondary` | 1,603 | 161 | 0 | `adjust-duo`→`adjust-1-duo`, `alarm-minus-duo`→`alarm-exclamation-duo` | — |
...
| `vscode-icons` | `VscodeIconsSecondary` | 326 | 64 | 0 | `file-type-apib2`→`file-type-apib`, `file-type-light-jsconfig`→`file-type-jsconfig` | — |
...
| `skill-icons` | `SkillIconsSecondary` | 125 | 37 | 0 | `ableton-light`→`ableton-dark`, `actix-light`→`actix-dark` | — |
...
| `streamline-sharp-color` | `StreamlineSharpColorSecondary` | 437 | 10 | 0 | `chat-bubble-disable-oval-flat`→`battery-empty-2-flat`, `cloud-off-flat`→`battery-empty-2-flat` | — |
...
| `flag` | `FlagSecondary` | 47 | 5 | 0 | `mc-1x1`→`id-1x1`, `mc-4x3`→`id-4x3` | — |
...
| `icon-park` | `IconParkSecondary` | 79 | 3 | 0 | `radio-two`→`inclusive-gateway`, `setting`→`preview-open` | — |
| `wordpress` | `WordpressSecondary` | 4 | 3 | 0 | `corner-bottom-right`→`corner-bottom-left`, `corner-top-left`→`corner-bottom-left` | — |
| `token-branded` | `TokenBrandedSecondary` | 203 | 3 | 0 | `hedera-hashgraph`→`hedera`, `milkomeda-c1`→`milkomeda-a1` | — |
...
| `glyphs-poly` | `GlyphsPolySecondary` | 293 | 2 | 0 | `male`→`female`, `map-signs`→`directions-sign` | — |
...
| `devicon` | `DeviconSecondary` | 180 | 1 | 0 | `amazonwebservices-wordmark`→`amazonwebservices` | — |
