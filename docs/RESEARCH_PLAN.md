...
1. Phase 1 (1 day): generator emits shards alongside existing
   `icons_index.json` — no client change. **[shipped — see commit
   "feat(web,generator): §11 per-pack shards"]**
2. Phase 2 (2 days): `PackBloc` switches to shards; search still uses
   ...
   visit a pack.** — current shipped infrastructure fans-out parallel
   shard fetch + assembles same `IconCatalog` API, so existing
   consumers don't need to change; future iteration can lazy-fetch per
   pack instead.
...
### Shipped infrastructure (as of this commit)

- `tools/generator/src/website_codegen.ts:buildIconShards()` emits one
  shard per pack at `cdn/icons-index/v1/<prefix>.json` + a shard
  manifest at `cdn/icons-index/v1/index.json` (sha256 + iconCount per
  pack). Determinism verified via unit tests
  (`website_codegen.test.ts`).
- `lib/bootstrap/cdn_config.dart` — `CdnManifest` + `CdnHttpClient`
  with timeout + fallback semantics. Dart unit tests in
  `test/cdn_config_test.dart`.
- `lib/bootstrap/icon_catalog.dart` — `IconCatalog.load()` and
  `PackIndex.load()` are CDN-aware behind the `kUseCdn` const; default
  `false` keeps every consumer behaviour identical to main. When
  flipped on, shard fan-out + parallel fetch happens before the
  existing `compute(_parse, ...)` worker pass — preserves the heavy
  parse path in a background isolate.
- `lib/cdn/` is `.gitignore`d for now; it gets regenerated by
  `bun run generate` and will be committed alongside the
...
- `buildCdnManifest()` in `website_codegen.ts` emits the routing
  manifest into `lib/data/cdn_manifest.json` (always bundled,
  ~250 B). Mirrors the same `packs.json` content to
  `lib/cdn/packs/v1/packs.json` (byte-identical — verified
  empirically and in the unit tests).
- `lib/bootstrap/icon_catalog.dart:PackIndex.load()` now consults
  `kUseCdn`. With `false` (default) it reads the legacy bundled
  asset; with `true` it does `client.getString(manifest.packsJsonUrl)`
  with a `Duration(seconds: 15)` timeout and falls back to the
  bundled asset on any error.
- Generator pubspec template auto-adds `http: ^1.2.0` and the
