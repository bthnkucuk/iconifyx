...
- `buildCdnManifest()` in `website_codegen.ts` emits the routing
  ...
  empirically and in `website_codegen.test.ts`).
...
- `lib/bootstrap/icon_catalog.dart:PackIndex.load()` now consults
  ...
  with a 15 s timeout and falls back to the bundled asset on any
  error.
...
once the CDN tree is tagged on jsDelivr. The §11 per-pack shards
land in a follow-up commit that uses the same
`CdnManifest.iconsIndexPath` already plumbed through the routing
manifest.
